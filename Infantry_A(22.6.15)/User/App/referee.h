...
typedef _packed struct	//0x020E 雷达易伤效果
												//0-1：雷达触发双倍易伤的机会
												//2：是否触发双倍易伤
{
 uint8_t radar_info;
...
typedef _packed struct	//0x0110 校验数据
{
graphic_data_struct_t grapic_data_struct;
uint8_t data[30];
} ext_client_custom_character_t;

typedef _packed struct	//0x0120 哨兵自主决策指令
{
uint32_t sentry_cmd; 
} sentry_cmd_t;

typedef _packed struct	//0x0121 雷达自主决策指令
{
uint8_t radar_cmd;
} radar_cmd_t;
/*  ---------------------------0x0301子内容ID-----------------------------------------*/

typedef _packed struct	//0x0303 选手下发坐标
{
...
typedef _packed struct		//0x0304 键鼠遥控数据同步
{
int16_t mouse_x;
int16_t mouse_y;
int16_t mouse_z;
...
typedef _packed struct	//0x0305 小地图接受坐标
{
...
typedef _packed struct	//0x0307 哨兵移动路径点
{
uint8_t intention;
...
typedef _packed struct	//0x0308 自定义接受数据
{ 
uint16_t sender_id;
uint16_t receiver_id;
...
////暂时放在这里
//typedef __packed struct  //0x0301 绘制字符
//{
//graphic_data_struct_t grapic_data_struct;
//uint8_t data[30];
//} ext_client_custom_character_t;
